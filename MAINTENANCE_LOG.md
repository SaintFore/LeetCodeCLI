...
#### 完成的工作
1. **版本更新**: 更新所有文件版本号到1.4.0
   - `setup.py`: 1.1.0 → 1.4.0
   - `PKGBUILD`: 1.0.0 → 1.4.0
   - `leetcode-fsrs-cli-bin/PKGBUILD`: 1.0.0 → 1.4.0

2. **文档完善**: 更新README.md添加新特性概览和版本亮点
   - 添加特性概览和版本亮点
   - 更新徽章和项目状态
   - 突出自动化发布和依赖优化

3. **维护记录**: 更新本维护记录，记录1.4.0版本发布
...
## 📅 2025-11-28: v1.5.0 功能更新 - 真实认证与同步

### 变更内容
- **新增功能**:
  - 实现 `AuthManager`，支持 LeetCode Cookie 认证
  - 实现 `LeetCodeAPIClient`，支持 GraphQL API 调用
  - 实现 `SyncManager`，支持同步最近提交记录
  - 新增 CLI 命令: `auth login`, `auth logout`, `auth status`, `sync`
- **测试**:
  - 添加 `tests/test_real_cookie.py` 和 `tests/test_sync_real.py` 集成测试
  - 添加 `tests/test_sync_flow.py` 单元测试
- **文档**:
  - 更新 README.md 反映新功能
  - 更新 setup.py 版本号至 1.5.0

### 技术细节
- **Cookie处理**: 支持 `LEETCODE_SESSION` 和完整 Cookie 字符串
- **CSRF防护**: 自动解析和处理 CSRF Token
- **数据同步**: 仅同步 "Accepted" 状态的提交，避免重复添加
